...
check_sat_response argosmt_solver_interface::start_solver(const std::vector<clause *> & clauses, formula_transformer * ft)
{
    ...
    /*trying to make sense - zebra*/
    std::cout << "We have reached start solver" << std::endl;
  ...
  for(unsigned i = 0; i < clauses.size(); i++){
    ...
    // zebra
    //std::cout << "Initial clause (?) " << *clauses[i] << std::endl;
  }
  ...
  if(s == "QF_UF")
    {
        // zebra
        std::cout << "We have reached line 121 in argosmt solver interface" << std::endl;
    ...
    // zebra - ovde se poziva resavanje :)
