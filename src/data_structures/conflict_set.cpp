...
clause * conflict_set::get_clause() const
{
  ...
  // zebra
//   std::cout << "Klauza nastala u conflict_set::get_clause() je ";
//   cl->print_clause(std::cout);
//   std::cout << "\nBroj literala u _conflicting " << _conflicting.size() << std::endl;
//     
//   std::cout << "Broj explained literala: " << _explained.size() << "\n Objasnjeni literali su: "<< std::endl;
//   
//   for(auto e : _explained)
//       std::cout << e << " ";
//   std::cout << std::endl;
...
clause * conflict_set::get_conflict_clause() const {
    unsigned current_unexplained = _next_to_explain;
    
    clause * cl = new clause();
    cl->reserve(_explained.size() - current_unexplained);
    
    while(current_unexplained < _explained.size()){
        cl->push_back(_solver.get_literal_data(_explained[current_unexplained])->get_opposite());
        current_unexplained++;
    }
