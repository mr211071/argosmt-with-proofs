...
{
    // zebra
    //std::cout << "We have reached add_initial_clauses" << std::endl;
...
void solver::initialize_solver()
{
    ...
    // zebra
    //std::cout << "We have reached initialize solver" << std::endl;
  ...
  for(unsigned i = 0; i < _initial_clauses.size(); i++)
      ...
      for(unsigned j = 0; j < _initial_clauses[i]->size(); j++)
	{
      ...
      //zebra 
      //std::cout << "Clause after changes: " << *_initial_clauses[i] << std::endl;      
    }
...
void solver::apply_conflict(const explanation & conflicting, theory_solver * conflict_ts)
{
  ...
  // zebra
  if(_produce_proofs)
      _conflict_clause = conflicting.get_clause();
...
void solver::apply_explain(const expression & l, const explanation & expl)
{
  ...
  if(_produce_proofs){
    //expression pivot = l;
    proof left = _proof;
    proof right = expl.get_clause()->get_proof();
    
    _proof = std::shared_ptr<proof_node>(new resolution_proof(_formula_transformer->expand_names(l), left, right));
  }
...
void solver::instantiate(const expression & l, const expression_vector & gterms)
{
  ...
  // zebra
  // odkomentarisali smo ovo ispod sa INSTANTIATING i OBTAINED
  
  /*std::cout << "We have reached instantiate" << std::endl;
...
check_sat_response solver::solve()
{
    ...
    // zebra
    std::cout << "We have reached line 817 in solver.cpp, method solve" << std::endl;
  ...
  while(true)
	...
	{	  
        ...
        // konfliktna klauza
        std::cout << "Konfliktna klauza je ";        
        // clause * cl = _conflict_set.get_conflict_clause(); // da li smo dobili konfliktnu klauzu??? da, ali posle ne mozemo da je nadjemo u spisku klauza 
        _conflict_clause->print_clause(std::cout);
        std::cout << std::endl;
        
        _proof = _conflict_clause->get_proof(); // ovo radi!!
        
        // treba da pronadjemo njen dokaz u inicijalnim klauzama
        
//         for(auto l : _initial_clauses)
//             l->print_clause(std::cout);
//         
//         auto it = std::find(_initial_clauses.begin(), _initial_clauses.end(), cl);
//         
//         if(it != _initial_clauses.end()){
//             int index = it - _initial_clauses.begin();
//             std::cout << "Index je " << index << std::endl;
//         }
//         else{
//             int index = _initial_clauses.end() - _initial_clauses.begin();
//             std::cout << "Nema konfliktne klauze medju pocetnim klauzama, it je " << index << std::endl;
//         }
        
//         std::cout << "Da li postoji " <<  << std::cout;
//         
//         for(unsigned i = 0; i < _initial_clauses.size(); i++){
//             if (_initial_clauses[i] == cl){
//                 proof p = _initial_clauses[i]->get_proof();
//                 std::cout << "Dokaz konfliktne klauze ";
//                 p->print_proof(std::cout);
//                 std::cout << std::endl;
//             }
//         }
...
//           std::cout << "Objasnjavamo literal " << l << std::endl;
//           std::cout << "Konfliktni set nakon toga ima " << _conflict_set.get_conflicting().size() << std::endl;
      ...
      // zebra - neki konfliktni skupovi ?
      unsigned velicina = conflicting.size();
      std::cout << "Linija 948 iz solver.cpp ";
	  ...
	  if(_conflict_set.last_level() != 0)
	  ...
	  else
	    {
          ...
          _empty_clause->set_proof(_proof);
	      //std::cout << "SIZE: " << _empty_clause->size() << std::endl;
          
          // zebra
//           std::cout << "Dosli smo do prazne klauze! Da vidimo koliko ima naucenih: ";
//           unsigned i = get_learnt_clauses().size();
//           std::cout << i << std::endl;
//           
//           std::cout << "Trail: ";
//           _trail.print_trail(std::cout);
//           std::cout << std::endl;
