					     ...
					     expression & name)
{
  ...
  if(it != _names.end())
    {
      // zebra
      std::cout << "cnf_transformation: it != _names.end(): " << it->second << std::endl;
  ...
  if(expr->is_not())
  ...
  else if(expr->is_and())
    {
	 ...
	 /\ (~s \/ sn) */
      ...
      for(expression_vector::const_iterator it = op_names.begin(), it_end = op_names.end(); it != it_end; ++it)
        {
            if(is_false(*it))
                false_present = true;
        }
      ...
      else
        {
            ...
            for(expression_vector::const_iterator it = op_names.begin(),
                it_end = op_names.end(); it != it_end; ++it)
                {
                    ...
                    if(_produce_proofs){                        
                        short_clause->set_proof(std::shared_ptr<proof_node>(new axiom_and_elim(expand_names(expr), i)));
                        i++;
                    }
            ...
            if(_produce_proofs){
                long_clause->set_proof(std::shared_ptr<proof_node>(new axiom_and_intro(expand_names(expr))));
            }
	 ...
	 (s1 \/ s2 \/ ... \/ sn \/ ~s) */
      ...
      if(true_present)
      ...
      else
        {
            ...
            for(expression_vector::const_iterator it = op_names.begin(), it_end = op_names.end(); it != it_end; ++it)
                {
                    ...
                    if(_produce_proofs){
                        short_clause->set_proof(std::shared_ptr<proof_node>(new axiom_or_intro(expand_names(expr), i)));
                        i++;
                    }
            ...
            if(_produce_proofs){
                long_clause->set_proof(std::shared_ptr<proof_node>(new axiom_or_elim(expand_names(expr))));
            }            
        ...
        else
        {
            ...
            for(expression_vector::const_iterator it = op_names.begin(),
                it_end = op_names.end(); it != it_end; ++it)
                {
                    ...
                    if(_produce_proofs){
                        short_clause->set_proof(std::shared_ptr<proof_node>(new axiom_impl_intro(expand_names(expr), i)));
                        i++;
                    }
            ...
            if(_produce_proofs){
                long_clause->set_proof(std::shared_ptr<proof_node>(new axiom_impl_elim(expand_names(expr))));
            }            
    ...
    {	
      ...
      if(op_names[0] == op_names[1] ||
      ...
      else 
        {
            ...
            if(_produce_proofs){
                cl->set_proof(std::shared_ptr<proof_node>(new axiom_eq2_elim(expand_names(expr))));
            }
            ...
            if(_produce_proofs){
                cl->set_proof(std::shared_ptr<proof_node>(new axiom_eq1_elim(expand_names(expr))));
            }
            ...
            if(_produce_proofs){
                cl->set_proof(std::shared_ptr<proof_node>(new axiom_eq1_intro(expand_names(expr))));
            }
            ...
            if(_produce_proofs){
                cl->set_proof(std::shared_ptr<proof_node>(new axiom_eq2_intro(expand_names(expr))));
            }
...
void formula_transformer::top_level_cnf_transformation(const expression & expr, 
						       std::vector<clause *> & clauses)
{
   std::cout << "Pocinjemo sa " << clauses.size() << " i izrazom "; 
  ...
  if(is_false(expr))
    {
      ...
      if(_produce_proofs){
          proof false_p = std::shared_ptr<proof_node>(new axiom_false());
          cl->set_proof(false_p);
      }
  ...
  if(_produce_proofs){
      proof assume = std::shared_ptr<proof_node>(new proof_assume(expand_names(name)));
      name_unit->set_proof(assume);
  }
  
  clauses.push_back(name_unit);
  
  std::cout << "Top level CNF\nTrenutna klauza: ";
  name_unit->print_clause(std::cout);
  std::cout << " Dokaz klauze: ";
  name_unit->get_proof()->print_proof(std::cout);
  std::cout << std::endl;
  
  std::cout << "Trenutni broj klauza: " << clauses.size() << std::endl;
   
   for(auto i : clauses){
       std::cout << "Klauza: " << *i << " i njen dokaz ";
       i->get_proof()->print_proof(std::cout);
       std::cout << std::endl;
   }
